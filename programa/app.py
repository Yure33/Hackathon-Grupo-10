...
Arquitetura com Atualização de Estado em Tempo Real e Relatório de Auditoria.

Regra de Negócio:
    - Transição DIREITA -> ESQUERDA (Lado A -> Lado B): ENTRADA (+1 no RU)
    - Transição ESQUERDA -> DIREITA (Lado B -> Lado A): SAÍDA (-1 no RU)
"""
...
class Config:
    ...
    max_lost_seconds: float = 1.2  # Tolerância temporal para perda de rastreio
    reid_max_dist_ratio: float = 0.12  # Distância máxima para re-identificação
    min_hits: int = 3  # Número mínimo de detecções para validar o objeto
...
class RealTimeEntryExitCounter:
    ...
    def update(self, frame_idx: int, dets: list[tuple[int, float, float]]) -> tuple[dict[int, int], list[str]]:
        """
        Processa as detecções do frame atual e avalia transições instantâneas de zona.
        Retorna a atribuição de IDs e uma lista de mensagens de log geradas neste frame.
        """
        n = 5
        ...
        # 1) Associação de IDs conhecidos da YOLO
        for tid, cx, cy in dets:
            ...
            if lid is not None and lid in self.active:
                ...
                if trk.last_frame != frame_idx:
                    ...
                    # AVALIAÇÃO DE TRANSIÇÃO EM TEMPO REAL
                    if trk.current_side != side_atual and trk.hits >= self.cfg.min_hits:
                        if trk.current_side == "A" and side_atual == "B":
                            self.count += 1
                            evento = "ENTROU"
                            msg = f"🚨 [ENTRADA] Pombo Lógico #{trk.lid} (YOLO ID #{tid}) cruzou A -> B. Total no RU: {self.count}"
                        elif trk.current_side == "B" and side_atual == "A":
                            self.count = max(0, self.count - 1)
                            evento = "SAIU"
                            msg = f"🚨 [SAÍDA] Pombo Lógico #{trk.lid} (YOLO ID #{tid}) cruzou B -> A. Total no RU: {self.count}"
                        else:
                            evento = "INDEFINIDO"
                            msg = ""
        ...
        # 2) Re-associação de IDs perdidos (Recuperação de Glitch)
        ...
        for i, (tid, cx, cy) in enumerate(new_dets):
            ...
            log_novo = f"ℹ️ [NOVO RASTREIO] Novo pombo identificado: ID Lógico #{lid} no Lado {side_inicial} (Frame {frame_idx})"
...
def run(args) -> None:
    ...
    print(f"  • SALDO ACUMULADO NO RU:         {counter.count}")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    p.add_argument("--video", default="videos/pigeon-video-1.mp4", help="Arquivo de vídeo de entrada")
